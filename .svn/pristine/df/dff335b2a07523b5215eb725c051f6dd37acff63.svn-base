...
@WebServlet("/updateProductRegistration.do")
@MultipartConfig(fileSizeThreshold=1024*1024*10, 
maxFileSize=1024*1024*30, maxRequestSize=1024*1024*50)
public class UpdateProductRegistration extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		proVo.setPro_num(listNum);
		
		
		ICFMSService service = CFMSServiceImpl.getInstance();
		
		
		int cnt = service.updateProlist(listVo);
		
		request.setAttribute("result", cnt);
		
		int cnt2 = service.updateProdlist(proVo);
